/**
 * Browser-facing proxy for PATCH /alerts/{anomaly_id}.
 *
 * Forwards the NextAuth session token to FastAPI as Bearer. Keeps BACKEND_URL
 * out of the browser bundle. Used by the acknowledge / dismiss controls in the
 * alerts table.
 */
...
export async function PATCH(
  ...
  // Body passed through verbatim — backend Pydantic validates.
  let body: unknown
  try {
  ...
  try {
    ...
    console.error("[/api/alerts/[id] PATCH] Network error:", err)
