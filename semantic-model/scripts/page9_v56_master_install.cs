...
@"VAR _rte =
    CALCULATE(
        AVERAGE( gold_battery_dispatch[round_trip_efficiency] ),
        gold_battery_dispatch[is_simulated] = FALSE()
    )
RETURN
    DIVIDE( _rte, 100 )",
...
Output("Page 9 v56 master install COMPLETE. Save model (Ctrl+S), then refresh Power BI report.");
