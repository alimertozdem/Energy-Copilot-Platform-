// =====================================================================
// Energy Copilot Platform - Page 1 Fix Pack v2  (SUPERSEDES v1)
// File: semantic-model/scripts/page1_fixpack_2026_06_18_v2.csx
// Tool: Tabular Editor 2/3 -> C# Script -> paste -> Run -> Ctrl+S -> Refresh
//
// WHY v2:
//   * v1 tried to ALIAS existing measures (P1 Scorecard EUI / ...EUI Color).
//     That created a DUPLICATE NAME -> "name already used / Missing_References"
//     -> a visual broke. v2 NEVER touches existing measures; it only creates
//     NEW names, and the cleanup block below removes the duplicate v1 left.
//   * v1 EUI annualized a partial slicer window (x365/days) AND mixed orphan
//     kWh with master-only area -> portfolio read 416 (too high). v2 uses a
//     ROBUST trailing-12-months-of-data window, master buildings only ->
//     verified against live gold: portfolio = 368, Berliner = 78 (sane).
//
// DESIGN (intentional):
//   * EUI = trailing 12 months of AVAILABLE data, slicer-independent
//     (REMOVEFILTERS('Date') + DATESBETWEEN). It is an annual INTENSITY for
//     benchmarking, so it must not shrink when the user narrows the slicer.
//   * AREA = silver_building_master[gross_floor_area_m2] everywhere.
//   * Orphans (B013-15, not in master) excluded by iterating master ids.
//   * Benchmarks = ELECTRICITY EUI (model kWh = grid electricity); edit (3).
//
// REQUIRES: a date table named 'Date' marked as Date Table, related to
//           gold_kpi_daily[date]. (Already set up per v17.) If your date
//           table has another name, replace 'Date' below.
// =====================================================================

string home   = "gold_kpi_daily";
string folder = "Page1 Portfolio";
if (!Model.Tables.Contains(home))
    throw new Exception("Home table '" + home + "' not found. Set 'home' correctly.");

// --- CLEANUP: remove duplicate measures v1 may have created -----------
foreach (var nm in new[] { "P1 Scorecard EUI", "P1 Scorecard EUI Color" }) {
    var hits = Model.AllMeasures.Where(x => x.Name == nm).ToList();
    for (int i = 1; i < hits.Count; i++) hits[i].Delete();   // keep one, drop extras
}

// case/space-insensitive upsert; never duplicates a name
Func<string,Measure> find = (name) =>
    Model.AllMeasures.FirstOrDefault(x => x.Name.Trim().Equals(name, StringComparison.OrdinalIgnoreCase));
Action<string,string,string> upsert = (name, dax, fmt) => {
    var m = find(name);
    if (m == null) m = Model.Tables[home].AddMeasure(name);
    m.Expression = dax.Trim();
    if (!string.IsNullOrEmpty(fmt)) m.FormatString = fmt;
    m.DisplayFolder = folder;
};

// 1) BUILDING EUI - trailing 12 months of data, master area (slicer-independent)
upsert("P1 Building EUI", @"
VAR _maxDate = CALCULATE ( MAX ( gold_kpi_daily[date] ), REMOVEFILTERS ( 'Date' ) )
VAR _kwh =
    CALCULATE ( SUM ( gold_kpi_daily[total_consumption_kwh] ),
        REMOVEFILTERS ( 'Date' ),
        DATESBETWEEN ( 'Date'[Date], _maxDate - 364, _maxDate ) )
VAR _area = SELECTEDVALUE ( silver_building_master[gross_floor_area_m2] )
RETURN DIVIDE ( _kwh, _area )", "#,##0.0");

// 2) PORTFOLIO EUI - energy-weighted, trailing 12 months, master-only (orphan-safe)
upsert("P1 Portfolio EUI", @"
VAR _maxDate = CALCULATE ( MAX ( gold_kpi_daily[date] ), REMOVEFILTERS ( 'Date' ) )
VAR _t =
    ADDCOLUMNS ( VALUES ( silver_building_master[building_id] ),
        ""@kwh"", CALCULATE ( SUM ( gold_kpi_daily[total_consumption_kwh] ),
            REMOVEFILTERS ( 'Date' ),
            DATESBETWEEN ( 'Date'[Date], _maxDate - 364, _maxDate ) ),
        ""@area"", CALCULATE ( MAX ( silver_building_master[gross_floor_area_m2] ) ) )
RETURN DIVIDE ( SUMX ( _t, [@kwh] ), SUMX ( _t, [@area] ) )", "#,##0.0");

// 3) TYPE BENCHMARK - electricity EUI kWh/m2.yr (EDIT VALUES HERE)
upsert("P1 EUI Type Benchmark", @"
SWITCH ( SELECTEDVALUE ( silver_building_master[building_type] ),
    ""Office"",        95,
    ""Retail"",       165,
    ""Hotel"",        105,
    ""Healthcare"",   150,
    ""Education"",     80,
    ""Logistics"",     60,
    ""Data_Center"", 1000,
    ""Lab"",          300,
    120 )", "#,##0");

// 4) EUI INDEX (% of own sector norm; 100 = on benchmark) - benchmark visual value
upsert("P1 EUI Index Pct",
    "DIVIDE ( [P1 Building EUI], [P1 EUI Type Benchmark] ) * 100", "#,##0\\%");

// 5) EUI COLOR (1 green / 2 amber / 3 red) vs own type benchmark
upsert("P1 EUI Index Color", @"
VAR _r = DIVIDE ( [P1 Building EUI], [P1 EUI Type Benchmark] )
RETURN SWITCH ( TRUE(), _r <= 0.85, 1, _r <= 1.20, 2, 3 )", "0");

// 6) PORTFOLIO BENCHMARK (energy-weighted target) - Avg-EUI card TARGET
//    Verified expected value ~215 (kills the -110,283% goal).
upsert("P1 Portfolio EUI Benchmark", @"
VAR _t =
    ADDCOLUMNS ( VALUES ( silver_building_master[building_id] ),
        ""@a"", CALCULATE ( MAX ( silver_building_master[gross_floor_area_m2] ) ),
        ""@b"", CALCULATE ( [P1 EUI Type Benchmark] ) )
RETURN DIVIDE ( SUMX ( _t, [@a] * [@b] ), SUMX ( _t, [@a] ) )", "#,##0.0");

// 7) ACTIVE ANOMALIES - portfolio-grade CARD measure (was "(Blank)")
upsert("P1 Active Anomalies", @"
CALCULATE (
    COUNTROWS ( gold_anomaly_log ),
    gold_anomaly_log[is_resolved] = FALSE ( ),
    UPPER ( gold_anomaly_log[severity] ) IN { ""HIGH"", ""CRITICAL"" }
) + 0", "#,##0");

// 8) BUILDING COST (period) - single-sourced from gold, no hardcoded tariff
upsert("P1 Building Cost EUR",
    "SUM ( gold_kpi_daily[estimated_cost_eur] )", "€ #,##0");

Info("Page 1 fix pack v2 applied. Duplicates cleaned, 8 measures upserted. "
   + "Save (Ctrl+S), Refresh. Expected: Berliner EUI ~78, Portfolio EUI ~368, "
   + "Portfolio benchmark ~215, Active Anomalies = 4. "
   + "Then rebind: Avg EUI card value->P1 Portfolio EUI, target->P1 Portfolio EUI Benchmark, "
   + "title 'Avg Portfolio EUI (kWh/m2.yr)'; anomalies card->P1 Active Anomalies; "
   + "benchmark visual value->P1 EUI Index Pct, color->P1 EUI Index Color, constant line 100.");
