...
# ── v5 Physics-Aware Per-Building Overrides (2026-05-14) ──────────────────────
# Each building has a unique envelope, equipment age and HVAC topology.
# A flat "building-type density" produced EUI inconsistencies vs. building
# physical properties:
#   - B002 Retail (U-wall 0.80, old + gas+AC, fluor): under-reported EUI 161 → target ~250
#   - B006 Education (U-wall 0.90, 1985, no LED, VRF): under-reported EUI 47 → target ~200
# v5 introduces per-building overrides that scale electricity demand to match the
# bina-spesifik thermal envelope, lighting tech and HVAC system.
#
# Fields:
#   density          : occupied-hour kW/m²  (lighting + equipment)
#   base             : unoccupied kW/m² (always-on services)
#   heat_coef_mult   : envelope-quality scaler for heating temperature response
#                      (relative to baseline U_wall=0.30 W/m²K → mult=1.00)
#   cool_coef_mult   : same for cooling — accounts for climate + envelope leakage
#                      (Istanbul retail leaks more cooling than Berlin office)
# Calibration target → resulting EUI ranges (kWh/m²/yr):
BUILDING_OVERRIDES = {
    # B001 Office (HP+PV, LED, U-wall 0.28) — target ~85 (modern GEG)
    "B001": {"density": 0.020, "base": 0.0030, "heat_coef_mult": 0.60, "cool_coef_mult": 1.00},
    # B002 Retail (gas+AC, fluor, U-wall 0.80, Istanbul Csa) — target ~250 (old retail, hot climate)
    # v5.1 (2026-05-14): raised density 0.045→0.055, cool_mult 1.8→2.4 to reach 230-260 EUI band
    "B002": {"density": 0.055, "base": 0.0090, "heat_coef_mult": 2.40, "cool_coef_mult": 2.40},
    # B003 Logistics (gas+PV+diesel, LED, U-wall 0.22, cold storage hints) — target ~125
    "B003": {"density": 0.025, "base": 0.0040, "heat_coef_mult": 0.50, "cool_coef_mult": 1.10},
    # B004 Hotel (district, mixed, U-wall 0.45, 24/7) — target ~290
    "B004": {"density": 0.035, "base": 0.0120, "heat_coef_mult": 1.30, "cool_coef_mult": 1.00},
    # B005 Healthcare (gas+diesel, LED, U-wall 0.50, 24/7 critical) — target ~450
    "B005": {"density": 0.055, "base": 0.0250, "heat_coef_mult": 1.40, "cool_coef_mult": 1.10},
    # B006 Education (VRF, fluor, U-wall 0.90, 1985 envelope) — target ~200
    # v5.1 (2026-05-14): density 0.032→0.050, base 0.006→0.013 (frost protection),
    # heat_mult 2.8→3.6 (envelope leak + VRF COP 2.5 vs HP 3.8)
    "B006": {"density": 0.050, "base": 0.0130, "heat_coef_mult": 3.60, "cool_coef_mult": 0.80},
}
...
def generate_energy_readings(building, ts):
    ...
    v5 (2026-05-14): Physics-aware per-building overrides replace the flat
    building-type density. Each building uses its own (density, base) load and
    a heat/cool coefficient scaled by its envelope U-values. This eliminates
    the previous EUI inconsistency where U-value 0.9 buildings under-reported
    consumption (Amsterdam Education 1985: 47 kWh/m²/yr → 200 kWh/m²/yr).
    """
    ...
    # v5: per-building override with fallback to building-type defaults
    _TYPE_DENSITY = {
        "Office": 0.020, "Retail": 0.040, "Logistics": 0.022,
        "Hotel": 0.035, "Healthcare": 0.055, "Education": 0.018,
    }
    ov = BUILDING_OVERRIDES.get(bid, {})
    density        = ov.get("density",         _TYPE_DENSITY[btype])
    base_intensity = ov.get("base",            0.003)
    heat_mult      = ov.get("heat_coef_mult",  1.0)
    cool_mult      = ov.get("cool_coef_mult",  1.0)

    base_kw  = area * base_intensity      # Always-on load (v5: per-building)
    peak_kw  = area * density             # Occupied load    (v5: per-building)
    ...
    # v5: Temperature-driven HVAC adjustment scaled by envelope quality.
    # heat_coef baseline = 0.015 per °C below 15°C → +1.5% extra demand/°C
    # heat_mult = 2.20 (B002: U=0.80) → effective 0.033/°C → 33% extra demand at 5°C
    # heat_mult = 0.50 (B003: U=0.22) → effective 0.0075/°C → only 7.5% extra at 5°C
    heat_eff = np.where((temp < 15) & occ, 1 + (15 - temp) * 0.015 * heat_mult, 1.0)
    cool_eff = np.where((temp > 25) & occ, 1 + (temp - 25) * 0.013 * cool_mult, 1.0)
    ...
    # Education: vacation reduced load (security, servers, frost-protection heating)
    # v5 fix: summer vacation drops to ~50% of base (servers + summer maintenance);
    # Christmas drop is smaller because frost-protection heating must run on poor-envelope
    # buildings (B006 U-wall 0.9 cannot freeze its pipes). Previous 0.35 multiplier
    # collapsed Education EUI by ~60%.
    if btype == "Education":
        month = ts.month.values
        day   = ts.day.values
        is_summer = (month >= 7) & (month <= 8)
        is_xmas   = ((month == 12) & (day >= 22)) | ((month == 1) & (day <= 8))
        # Summer: 0.55 × base (no heating, only servers + minimal lighting)
        demand = np.where(is_summer, base_kw * 0.55 * np.random.uniform(0.9, 1.1, n), demand)
        # Xmas: 0.85 × base (frost protection heating runs heavy if envelope poor)
        demand = np.where(is_xmas, base_kw * 0.85 * np.random.uniform(0.9, 1.1, n), demand)
...
def _columns_from_first_row():
    return list(BUILDINGS[0].keys())


def main():
    ts = make_timestamps()
    print(f"[gen v5] timestamps: {len(ts):,} intervals")

    bm_df = pd.DataFrame(BUILDINGS, columns=_columns_from_first_row())
    bm_df.to_csv(os.path.join(OUTPUT_DIR, "building_master.csv"), index=False)

    energy_frames = [generate_energy_readings(b, ts) for b in BUILDINGS]
    pd.concat(energy_frames, ignore_index=True).to_csv(
        os.path.join(OUTPUT_DIR, "raw_energy_readings.csv"), index=False)

    solar_frames = [generate_solar_generation(b, ts) for b in BUILDINGS if b.get("has_pv")]
    solar_frames = [df for df in solar_frames if df is not None]
    if solar_frames:
        pd.concat(solar_frames, ignore_index=True).to_csv(
            os.path.join(OUTPUT_DIR, "raw_solar_generation.csv"), index=False)

    bat_frames = [generate_battery_status(b, ts) for b in BUILDINGS if b.get("has_battery")]
    bat_frames = [df for df in bat_frames if df is not None]
    if bat_frames:
        pd.concat(bat_frames, ignore_index=True).to_csv(
            os.path.join(OUTPUT_DIR, "raw_battery_status.csv"), index=False)

    w_frames = [generate_weather_data(b, ts) for b in BUILDINGS]
    pd.concat(w_frames, ignore_index=True).to_csv(
