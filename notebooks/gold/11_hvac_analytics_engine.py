...
df_bldg = df_bldg.withColumn(
    ...
    when(col("system_type") == "heat_pump",
         when(col("cop_rated") >= 4.5, lit("ASHP İnverter (Premium)"))
         .when(col("cop_rated") >= 4.0, lit("ASHP İnverter (Yüksek Verim)"))
         .when(col("cop_rated") >= 3.0, lit("ASHP Standart"))
         .otherwise(lit("Isı Pompası (Hava Kaynaklı)"))
    )
    .when(col("system_type") == "gshp",     lit("GSHP (Toprak Kaynaklı HP)"))
    .when(col("system_type") == "wshp",     lit("WSHP (Su Kaynaklı HP)"))
    .when(col("system_type") == "hybrid",   lit("Hibrit HP + Gaz Kazan"))
    .when(col("system_type") == "gas_boiler", lit("Gaz Yoğuşmalı Kazan"))
    .when(col("system_type") == "vrf",      lit("VRF/VRV Sistemi"))
    .when(col("system_type") == "district", lit("Bölgesel Isıtma/Soğutma"))
    .when(col("system_type") == "biomass",  lit("Biyokütle Kazanı"))
    .when(col("system_type") == "electric", lit("Elektrikli Isıtma"))
    .when(col("system_type") == "chiller",  lit("Chiller (Soğutma Odaklı)"))
    .otherwise(lit("Bilinmiyor / Diğer"))
)
...
df_with_reno = df_with_eff.withColumn(
    ...
    when(
        ...
        lit("Gaz ısıtma + kötü yalıtım: ASHP geçişi + yalıtım yenileme öncelikli (CSRD Scope 1 riski)")
    ).when(
        (col("system_type") == "gas_boiler") & (col("_ins_band") == "medium"),
        lit("Gaz ısıtma: Isı pompasına geçiş planlanmalı (2030 öncesi AB direktifi)")
    ).when(
        (col("system_type") == "gas_boiler") & (col("_ins_band") == "good"),
        lit("Yalıtım yeterli — gaz→HP geçişi için uygun an, düşük retrofit riski")
    ).when(
        col("system_type").isin("heat_pump","gshp","wshp"),
        when(col("_ins_band") == "poor",
             lit("Isı pompası mevcut: Yalıtım iyileştirilirse SCOP 15-25% artar")
        ).otherwise(
             lit("İyi performans: Yıllık COP takibi + filtre/kompresör bakımı")
        )
    ).when(col("system_type") == "vrf",
        when(col("_ins_band") == "poor",
             lit("VRF sistemi: Yalıtım zayıf, bölge kayıpları yüksek — zarf öncelikli")
        ).otherwise(
             lit("VRF: Soğutucu gaz sızıntı kontrolü + bölge dengeleme optimizasyonu")
        )
    ).when(col("system_type") == "district",
        when(col("_ins_band") == "poor",
             lit("Bölgesel ısı: Yalıtım iyileştir → ağ bağlantı kapasitesi optimize edilir")
        ).otherwise(
             lit("Bölgesel ısı: İç dağıtım ağı kayıplarını izle, ısı eşanjörü bakımı")
        )
    ).when(col("system_type") == "hybrid",
        lit("Hibrit: Gaz payını azalt, HP çalışma saatini artır — 2030 hedefi saf HP")
    ).when(col("system_type") == "biomass",
        when(col("_ins_band") == "poor",
             lit("Biyokütle + kötü yalıtım: Zarf önce, yakıt verimliliği düşük")
        ).otherwise(
             lit("Biyokütle: Yakıt tedarik güvenliği + emisyon izleme (PM2.5)")
        )
    ).when(col("system_type") == "electric",
