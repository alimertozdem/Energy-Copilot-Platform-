...
# =============================================================================
# P2 (IMPLEMENTED) — OPC-UA / SCADA  (IEC 62541 — Siemens/ABB/Honeywell PLCs; live client = Phase 2.5)
# =============================================================================

class OpcUaAdapter(IotAdapter):
    """
    OPC-UA (IEC 62541) is the protocol of premium SCADA / building automation
    (Siemens Desigo, ABB, Honeywell). A gateway or asyncua client reads each
    monitored node and hands us one DataValue dict:
      {
        "node_id": "ns=2;s=B005.Chiller1.COP",   # NodeId (namespace + identifier)
        "browse_name": "Chiller Coefficient of Performance",
        "value": 4.2,
        "eu_unit": "ratio",                       # EUInformation engineering unit
        "status_code": "Good",                    # StatusCode severity (name or uint32)
        "source_timestamp": "2026-05-07T14:30:00Z",
        "building_id": "B005",                    # optional; else parsed from node_id
        "location": "Central Plant"
      }

    SCOPE: this adapter normalizes one already-read DataValue (pure function),
    exactly like the BACnet/Modbus adapters assume an upstream driver. The live
    OPC-UA session (asyncua client, certificates, subscriptions, real PLC) is the
    edge gateway's job and remains Phase 2.5 / out of scope here.
    """

    # BrowseName / DisplayName keyword -> our sensor_type vocabulary
    OPCUA_SENSOR_TYPE_MAP = {
        "supply air": "HVAC_supply_temp", "return air": "HVAC_return_temp",
        "temp": "HVAC_temp", "temperature": "HVAC_temp",
        "humid": "humidity", "rh": "humidity",
        "co2": "CO2", "carbon dioxide": "CO2",
        "coefficient of performance": "chiller_cop", "cop": "chiller_cop",
        "chilled water": "chilled_water_temp",
        "vfd": "fan_vfd_speed", "fan speed": "fan_vfd_speed",
        "boiler": "boiler_eff", "efficiency": "boiler_eff",
        "pump": "pump_pressure", "filter": "filter_dp",
        "pv": "pv_ac_power", "generation": "pv_ac_power", "inverter": "pv_ac_power",
        "power": "power", "demand": "power", "energy": "power",
    }

    # OPC-UA StatusCode (string name) -> reading_quality %
    STATUS_CODE_MAP = {
        "GOOD": 100, "GOODCLAMPED": 90, "GOODLOCALOVERRIDE": 90,
        "UNCERTAIN": 50, "UNCERTAINLASTUSABLEVALUE": 40, "UNCERTAINSENSORNOTACCURATE": 40,
        "BAD": 10, "BADNOTCONNECTED": 0, "BADDEVICEFAILURE": 0,
        "BADOUTOFSERVICE": 0, "BADWAITINGFORINITIALDATA": 0, "BADSENSORFAILURE": 0,
    }

    def _map_sensor_type(self, browse_name: str) -> str:
        name = str(browse_name).lower()
        for kw, st in self.OPCUA_SENSOR_TYPE_MAP.items():
            if kw in name:
                return st
        return "unknown"

    def _building_from_node(self, node_id: str) -> str:
        """Parse building_id from a string NodeId, e.g. 'ns=2;s=B005.Chiller1.COP' -> 'B005'."""
        nid = str(node_id)
        # NodeId components are ';'-separated: ns=2;s=B005.Chiller1.COP
        # (must skip the 'ns=' namespace, whose substring also contains 's=')
        for part in nid.split(";"):
            part = part.strip()
            if part.startswith("s="):
                token = part[2:].replace("/", ".").split(".", 1)[0].strip()
                if token:
                    return token.upper()
        return "UNKNOWN"

    def _quality_from_status(self, status) -> int:
        """OPC-UA StatusCode -> 0-100. Accepts string name or uint32 (severity = top 2 bits)."""
        if isinstance(status, str):
            return self.STATUS_CODE_MAP.get(status.strip().upper(), 80)
        if isinstance(status, (int, float)):
            code = int(status)
            severity = (code >> 30) & 0x3   # 0=Good, 1=Uncertain, 2/3=Bad
            return {0: 100, 1: 50, 2: 10, 3: 10}.get(severity, 50)
        return 80

    def normalize(self, raw_data: dict) -> dict:
        value, unit = self._normalize_value(
            raw_data.get("value", 0.0),
            raw_data.get("eu_unit", raw_data.get("unit", "unknown"))
        )
        node_id = raw_data.get("node_id", raw_data.get("nodeId", ""))

        if "status_code" in raw_data:
            quality = self._quality_from_status(raw_data["status_code"])
        else:
            q = raw_data.get("quality", 80)
            quality = int(min(100, max(0, q))) if isinstance(q, (int, float)) \
                else self.STATUS_CODE_MAP.get(str(q).strip().upper(), 80)

        normalized = {
            "device_id":       raw_data.get("device_id", node_id or "unknown"),
            "building_id":     raw_data.get("building_id", self._building_from_node(node_id)),
            "sensor_type":     raw_data.get("sensor_type",
                                            self._map_sensor_type(
                                                raw_data.get("browse_name",
                                                             raw_data.get("display_name", "")))),
            "sensor_location": raw_data.get("location", raw_data.get("browse_name", "Unknown")),
            "reading_value":   value,
            "reading_unit":    unit,
            "source_protocol": "OPC-UA",
            "timestamp":       raw_data.get("source_timestamp",
                                            raw_data.get("server_timestamp",
                                                         raw_data.get("timestamp", self._utc_now_iso()))),
...
if __name__ == "__main__":
    ...
    print("\n=== OPC-UA Adapter (SCADA) ===")
    opcua_raw = {
        "node_id": "ns=2;s=B005.Chiller1.COP",
        "browse_name": "Chiller Coefficient of Performance",
        "value": 4.2, "eu_unit": "ratio",
        "status_code": "Good",
        "source_timestamp": "2026-05-07T14:30:00Z",
        "location": "Central Plant Room",
    }
    result = AdapterFactory.get("OPC-UA").normalize(opcua_raw)
    print(json.dumps(result, indent=2))
