...
# ── Residential model hooks (P1, 2026-06-04) ──────────────────────
# unit_count + common_area_m2: enable the residential unit grain at building
# level. Pulled from bronze when present (residential onboarding); NULL for
# commercial buildings → fully additive, zero impact on the existing pipeline.
df_bld_silver = (df_bld_silver
    .withColumn("unit_count",
        (col("unit_count") if "unit_count" in df_bld_bronze.columns else lit(None)).cast("int"))
    .withColumn("common_area_m2",
        (col("common_area_m2") if "common_area_m2" in df_bld_bronze.columns else lit(None)).cast("double"))
)
...
# =============================================================================
# BÖLÜM 8 — WEATHER  (DEVRE DIŞI — gerçek hava ayrı loader'da)
# =============================================================================
# 2026-06-01: Sentetik weather dönüşümü + silver_weather_clean YAZIMI buradan
# ÇIKARILDI. silver_weather_clean artık YALNIZCA 02_openmeteo_weather_loader.py
# tarafından gerçek Open-Meteo verisiyle (temperature_2m + shortwave_radiation)
# yazılır. Eskiden 02_silver'ın sentetik weather'ı loader'ın gerçek verisini
# EZİYORDU (çakışma) — bu yüzden kaldırıldı.
# Sıra: 02_silver_transformation -> 02_openmeteo_weather_loader -> 03_gold_kpi_engine.
print("ADIM 5/5 - Weather: silver_weather_clean artik 02_openmeteo_weather_loader'da (skip).")
...
# (weather Z-ORDER kaldırıldı — silver_weather_clean'i 02_openmeteo_weather_loader yazar/yönetir)
...
# HDD/CDD yıllık özet: weather artık 02_openmeteo_weather_loader'da yazılır;
# bu notebook (loader'dan ÖNCE koşar) silver_weather_clean'i okumaz. Loader
# kendi HDD/CDD + irradiance doğrulamasını basar.
print("🌡️  HDD/CDD + irradiance ozeti: 02_openmeteo_weather_loader ciktisina bakin.")
...
print(f"✅ Weather: silver_weather_clean 02_openmeteo_weather_loader'a devredildi (bu notebook yazmaz)")
