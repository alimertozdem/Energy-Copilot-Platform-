...
#   (building_id, day_of_week, hour_of_day) → occupancy_probability, estimated_headcount
#
# INPUTS
...
# ── Blend weights (v1.3.0 change) ───────────────────────────
# NEDEN DEĞİŞTİRİLDİ:
#   v1.2.0 formülü: base × (signal × 0.60 + 0.40)
#   Sentetik veriyle signal ≈ 0.5 (düz) olduğunda sonuç: base × 0.70
#   → Hastane %90 base → %63 çıkıyor (gerçekçi değil).
#
#   v1.3.0 formülü: base × (signal × 0.40 + 0.60)
#   Signal=0.5 → base × 0.80  (arketip değerinin %80'i korunuyor)
#   Signal=0.0 → base × 0.60  (minimum: arketip %60 ağırlıklı)
#   Signal=1.0 → base × 1.00  (maksimum: tam arketip değeri)
#   → Hastane %90 base → %72 (mesai) / %44 (gece base=0.55 → %44) ✅
BLEND_WEIGHT_DATA        = 0.40  # weight for consumption-derived signal
BLEND_WEIGHT_BASE        = 0.60  # weight for rule-based base profile (dominant)
MODEL_VERSION            = "v1.3.0"

# ── Max Occupants Lookup (fallback if column not in silver_building_master) ──
# Gerçekçi kişi sayısı hesabı için bina başına maksimum kapasite.
# Kaynaklar: EN 15232, ASHRAE 90.1, bina tipi başına m² referansları.
# silver_building_master'da max_occupants kolonu varsa o tercih edilir.
MAX_OCCUPANTS_FALLBACK = {
    "B001": 260,   # Office  5200m² → 1/20m²
    "B002": 700,   # Retail  8500m² → 1/10m² (staff + peak visitors)
    "B003": 80,    # Logistics 12000m² → 1/150m²
    "B004": 230,   # Hotel   7800m² → 130 rooms × 1.5 guests + 45 staff
    "B005": 750,   # Healthcare 15000m² → 500 staff + 250 patients
    "B006": 950,   # Education  9500m² → 800 students + 150 staff
}
...
if "max_occupants" in _bldg_raw.columns:
    log("  → max_occupants kolonu silver_building_master'da bulundu.")
    df_building = (
        _bldg_raw.select(
            "building_id",
            "building_type",
            F.coalesce(F.col("utc_offset_hours"), F.lit(0)).alias("tz_offset"),
            F.col("max_occupants").cast("integer").alias("max_occupants"),
        )
    )
else:
    log("  ⚠️ max_occupants kolonu bulunamadı — fallback dict kullanılıyor.")
    # Fallback: Python dict'ten Spark literal'a çevir
    fallback_rows = [(k, v) for k, v in MAX_OCCUPANTS_FALLBACK.items()]
    df_max_occ = spark.createDataFrame(fallback_rows, ["building_id", "max_occupants"])
    df_building = (
        _bldg_raw
        .select(
            "building_id",
        ...
        .join(df_max_occ, "building_id", "left")
        .withColumn("max_occupants", F.coalesce(F.col("max_occupants"), F.lit(100)))  # ultimate fallback
    )
...
# N6 FIX + v1.3.0 — Profile-modulated blend with raised base weight
# ─────────────────────────────────────────────────────────────────────────────
# PROBLEM (old formula):
...
df_blended_raw = (
    ...
    .withColumn(
        ...
        F.when(
            ...
            # Profile-modulated blend (v1.3.0):
            # base acts as a gate AND sets the amplitude ceiling.
            # With BLEND_WEIGHT_BASE=0.60: signal=0 → base×0.60, signal=1 → base×1.0
            # This keeps archetype patterns dominant while still allowing
            # consumption signal to modulate intensity (busy vs quiet weeks).
    ...
    # estimated_headcount: occupancy_probability × max_occupants (rounded to integer)
    # max_occupants comes from building join done in Cell 6 (df_with_base already has it via df_building join)
)

# Join max_occupants onto blended result (df_with_base has building_id + building_type from df_building)
df_blended = (
    df_blended_raw
    .join(F.broadcast(df_building.select("building_id", "max_occupants")), "building_id", "left")
    .withColumn(
        "estimated_headcount",
        F.greatest(
            F.lit(0),
            F.round(F.col("occupancy_probability") * F.col("max_occupants")).cast("integer")
        )
    )
    .select(
        "building_id",
...
if no_data_count > 0:
    ...
    df_no_data_buildings = (
        ...
        .withColumn(
            "estimated_headcount",
            F.greatest(F.lit(0),
                F.round(F.col("base_probability") * F.col("max_occupants")).cast("integer"))
        )
        .select(
            "building_id",
            ...
            F.col("estimated_headcount").cast("integer"),
            F.col("max_occupants").cast("integer"),
            "profile_source",
...
OUTPUT_SCHEMA = StructType([
    ...
    StructField("estimated_headcount",   IntegerType(),   True),   # v1.3.0: kişi sayısı
    StructField("max_occupants",         IntegerType(),   True),   # v1.3.0: bina kapasitesi
